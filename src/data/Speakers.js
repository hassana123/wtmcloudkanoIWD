import amina from "../assets/speakers/amina.jpg";
import amina2 from "../assets/speakers/amina2.jpg";
import saa from "../assets/speakers/sa'ada.jpg";
import mercy from "../assets/speakers/mercy.jpg";
import aminaL from "../assets/team/amina.jpeg";
import suliyat from "../assets/speakers/suliyat.jpg";
import rasheedat from "../assets/team/rash.jpg";
import hassy from "../assets/team/hassy.jpeg";
import zenab from "../assets/speakers/zenab.jpg";
export const speakers = [
  {
      id: 1,
      name: "Amina Lawal",
      role: "Software & DevOps Engineer",
      company: "G3women",
      image: aminaL,
      bio: "Amina Lawal is a Software & DevOps Engineer, Google Women Techmaker Ambassador, and social impact advocate dedicated to leveraging technology for social good. She's the founder of G3women, empowering ambitious women to reach their full potential.",
      topic: "Breaking Barriers in Tech – No Permission Needed",
      day: "IWD 2025 Kano",
      social: {
          twitter: "https://x.com/amiynarh",
          linkedin: "https://www.linkedin.com/in/aminalawalofficial/",
          email:"amynalawal@gmail.com"
      }
  },
  {
      id: 2,
      name: "Sa’adat Aliyu",
      role: "Founder",
      company: "Shamrock Innovations",
      image: saa,
      bio: "Sa’adat Aliyu is the founder of Shamrock Innovations, an innovation hub helping women and youth build sustainable businesses. She is a passionate advocate for digital technologies and gender-based violence prevention.",
      topic: "Future of Women in Tech – The Next 10 Years",
      day: "IWD 2025 Kano",
      social: {
          twitter: "https://x.com/IAmSoosy",
          linkedin: "https://www.linkedin.com/in/saadat-aliyu-9b4446157/",
          email:"soosyamoora@gmail.com"
      }
  },
  {
      id: 3,
      name: "Amina R. Suleiman",
      role: "Scientific Officer & Digital Entrepreneur",
      company: "Nigeria’s National Space Center",
      image: amina2,
      bio: "Amina R. Suleiman is a Scientific Officer at Nigeria’s National Space Center, a freelance graphic designer, and a digital entrepreneur. She has expertise in branding, digital marketing, and IT services.",
      topic: "Overcoming Bias & Building a Personal Brand in Tech",
      day: "IWD 2025 Kano",
      social: {
          linkedin: "https://www.linkedin.com/in/aminars",
          email: "amina.rabiu22@gmail.com",
      }
  },
  {
      id: 4,
      name: "Mercy Markus",
      role: "Software Engineer",
      company: "Microsoft",
      image: mercy,
      bio: "Mercy Markus transitioned from Civil Engineering to Data Science and now works as a Software Engineer at Microsoft. She specializes in building developer platforms for Mixed Reality experiences.",
      topic: "Soft Skills for Career Growth",
      day: "IWD 2025 Kano",
      social: {
          twitter: "https://x.com/MercyMarkus",
          github: "https://github.com/MercyMarkus",
          linkedin: "https://www.linkedin.com/in/mercymarkus/",
      }
  },
  {
      id: 5,
      name: "Amina Rabiu",
      role: "Product Designer",
      company: "Schoola & Amiron Ventures",
      image: amina,
      bio: "Amina Rabiu is a Product Designer passionate about creating seamless digital experiences. She has experience in web development and has volunteered with healthcare NGOs and mentored aspiring designers.",
      topic: "The Role of Design Thinking in Career Growth",
      day: "IWD 2025 Kano",
      social: {
          twitter: "#",
          linkedin: "#",
      }
  },
  {
    id: 6,
    name: "Suliyat Idris",
    role: "Instructional Design Strategist",
    company: "Edunovative Concept & Smartclicks Tech-Wellness",
    image: suliyat,
    bio: "Suliyat Idris is an Instructional Design Strategist dedicated to creating impactful learning experiences. She advocates for digital empowerment, participatory learning, and using tech for social good.",
    topic: "Tech for Social Good: How Girls Can Change the World with Technology",
    day: "IWD 2025 Kano",
    social: {
        twitter: "https://x.com/Ennygirl4succes",
        linkedin: "https://www.linkedin.com/in/suliyataaidris?lipi=urn%3Ali%3Apage%3Ad_flagship3_profile_view_base_contact_details%3BwDlExGeGRRuhUHYJyCZIDg%3D%3D",
    }
  },
  {
    id: 7,
    name: "Zenab Adam",
    role: "Education Consultant & Public Speaking Coach",
    company: "Pro Teach Consults",
    image: zenab,
    bio: "Zenab Adam is an Education Consultant specializing in Early Years, a Tedx Speaker, Life Coach, and Public Speaking Coach. She is the founder of Pro Teach Consults and has empowered 19 cohorts through her workshops on teaching and public speaking.",
    topic: "Leadership & Public Speaking for Women",
    day: "IWD 2025 Kano",
    social: {
        linkedin: "https://www.linkedin.com/in/zenab-adam-03809795/",
        instagram: "https://www.instagram.com/zenaba.adam/",
        email: "adamszenab@gmail.com"
    }
  },
  {
    id: 8,
    name: "Rasheedat Atinuke Jamiu",
    role: "DevOps Engineer",
    company: "#",
    image: rasheedat,
    bio: "Rasheedat Atinuke Jamiu is a skilled DevOps Engineer specializing in scalable, reliable cloud infrastructures, automation, and observability. She is an active member of Google Women Techmakers Cloud Kano.",
    topic: "Guide to Serverless Computing with Google Cloud Platform",
    day: "IWD 2025 Kano",
    social: {
        linkedin: "https://www.linkedin.com/in/rasheedat-atinuke-jamiu/",
        instagram: "https://www.instagram.com/rassh_raj/?locale=zh_tw&hl=ar"
    }
  },
  {
    id: 9,
    name: "Hassana Abdullahi",
    role: "Frontend Engineer",
    company: "#",
    image: hassy,
    bio: "Hassana Abdullahi is a Frontend Engineer and Women Techmakers Ambassador specializing in performance optimization, authentication, and modern frontend technologies. She is passionate about building scalable, user-friendly web applications.",
    topic: "Frontend Power-Up: Speed, Storage & Beyond!",
    day: "IWD 2025 Kano",
    social: {
        linkedin: "https://www.linkedin.com/in/hassana-abdullahi-858040240/",
        twitter: "https://twitter.com/techSultana",
        email: "hassanaabdll1@gmail.com",
    }
  }
];
